(function($)
{
    $.Zebra_Tooltips = function(elements, options)
    {
        var defaults = {
            ...
            default_position: 'below',      // 位置, 可能的值是 'above' 和 'below'
            hide_delay: 100,                // 隐藏延迟
            keep_visible: true,
        ...
        /**
         *  生成工具提示的HTML代码并将其插入到DOM中
         *  返回包含工具提示组件的引用的对象
         *
         *  如果工具提示已经存在，该方法将只返回对象的引用的工具提示组件
         *
        ...
        var _create_tooltip = function($element)
        {
            ...
            if(!tooltip_info.tooltip)
            {
                ...
                var
                    ...
                    // 添加工具提示的内容
                    // 使用作为参数实例化对象的消息是, 或父元素的'title'属性中包含的信息
            ...
            if(tooltip_info.window_resized || tooltip_info.window_scrolled)
            {
                ...
                if(tooltip_left < horizontal_scroll)
                {
                    ...
                    // 如果没有空间显示箭头, 隐藏它
                    if(tooltip_left + arrow_left + (tooltip_info.arrow.width / 2) > tooltip_info.element_left + tooltip_info.element_width) arrow_left = -10000;
                };

                // 默认情况下, 我们假定工具提示位于元素上方, 因此箭头位于工具提示的底部
                ...
                tooltip_info.hide_timeout = setTimeout(function()
                {
                    ...
                    if(tooltip_info.tooltip)
                    {
                        ...
                        if(plugin.settings.onBeforeHide && typeof plugin.settings.onBeforeHide == 'function')
                            // 执行回调函数
                            plugin.setting.onBeforeHide($element, tooltip_info.tooltip);
                        ...
                        if(tooltip_info.destroy)
                            // 现在设置此标志, 以便用户快速地在元素消失时, 不再显示工具提示
