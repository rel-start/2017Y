...
; (function (window) {	// 还没封装好
	...
	scrollBarFx.prototype._calcBar = function () {
		this.yAxisRoll();
		if (this.options.xAxis) this.xAxisRoll();
	};
	...
	scrollBarFx.prototype.yAxisRoll = function () {
		var _opts = this.options;
		this.y_displaySize = _opts.displayArea.clientHeight;
		this.y_contentSize = _opts.contentArea.offsetHeight;
		_opts.y_barWrap.style.opacity = this.y_contentSize > this.y_displaySize ? 1 : '';

		// 每次计算滚动条回到顶部
		this.y_barPos = 0;
		_opts.y_bar.style.top = 0;
		_opts.contentArea.style.top = 0;

		this.y_oldDisplaySize = this.y_displaySize;

		if (this.y_contentSize <= this.y_displaySize) return;

		this.y_barWrap = _opts.y_barWrap.clientHeight;
		// 正常滚动条的高度
		this.y_norbar = this.y_displaySize / this.y_contentSize * this.y_barWrap;
		// 当前滚动条的高度。最大高度 barParent 的高度
		this.y_curbar = Math.min(this.y_norbar, _opts.maxHeight);
		// 滚动条最小高度 30px
		this.y_curbar = Math.max(this.y_curbar, _opts.minHeight);

		// 手动设置滚动条后的一个差值
		this.y_cbarRatio = (this.y_barWrap - this.y_norbar) / (this.y_barWrap - this.y_curbar);
		// 默认的滚动条的滚动路程 与 内容滚动距离的比值 / 手动设置的差值
		this.y_barRatio = this.y_norbar / this.y_displaySize / this.y_cbarRatio;
		_opts.y_bar.style.height = this.y_curbar + 'px';
	};

	scrollBarFx.prototype.xAxisRoll = function (leftNotZero) {
		var _this = this, _opts = _this.options;
		_this.x_displaySize = _opts.displayArea.clientWidth;
		_this.x_contentSize = _opts.contentArea.offsetWidth;
		_opts.x_barWrap.style.opacity = _this.x_contentSize > _this.x_displaySize ? 1 : '';

		_this.x_barPos = 0;
		_opts.x_bar.style.top = 0;
		_opts.contentArea.style.left = 0;

		_this.x_oldDisplaySize = _this.x_displaySize;

		if (_this.x_contentSize <= _this.x_displaySize) return;

		_this.x_barWrap = _opts.x_barWrap.clientWidth;
		// 正常滚动条的高度
		_this.x_norbar = _this.x_displaySize / _this.x_contentSize * _this.x_barWrap;
		// 当前滚动条的高度。最大高度 barParent 的高度
		_this.x_curbar = Math.min(_this.x_norbar, _opts.maxWidth);
		// 滚动条最小高度 30px
		_this.x_curbar = Math.max(_this.x_curbar, _opts.minWidth);

		// 手动设置滚动条后的一个差值
		_this.x_cbarRatio = (_this.x_barWrap - _this.x_norbar) / (_this.x_barWrap - _this.x_curbar);
		// 默认的滚动条的滚动路程 与 内容滚动距离的比值 / 手动设置的差值
		_this.x_barRatio = _this.x_norbar / _this.x_displaySize / _this.x_cbarRatio;
		_opts.x_bar.style.width = _this.x_curbar + 'px';
	};

	scrollBarFx.prototype.rollerEvent = function () {
		var _this = this, _opts = _this.options;
		_this.mouseWheel(_opts.displayArea, function upFn(e, rate) {
			if (_this.y_contentSize <= _this.y_displaySize) return;

			e.preventDefault();
			// 滚动距离
			_this.y_barPos -= _opts.speed * rate;
			// 边界判断
			if (_this.y_barPos < 0) _this.y_barPos = 0;
			// 滚动条的运动
			_opts.y_bar.style.top = _this.y_barPos + 'px';
			// 内容的滚动
			_opts.contentArea.style.top = -_this.y_barPos / _this.y_barRatio + 'px';
			...
			_this.y_barPos += _opts.speed * rate;
			// 边界判断
			if (_this.y_barPos > _this.y_barWrap - _this.y_curbar) _this.y_barPos = _this.y_barWrap - _this.y_curbar;
			// 滚动条的运动
			_opts.y_bar.style.top = _this.y_barPos + 'px';
			// 内容的滚动
			_opts.contentArea.style.top = -_this.y_barPos / _this.y_barRatio + 'px';
	...
	scrollBarFx.prototype.drag = function () {
		...
		mDrag({
			...
			disableX: true,
			moveFn() {
				_this.y_barPos = _opts.y_bar.offsetTop;
				_opts.contentArea.style.top = -_this.y_barPos / _this.y_barRatio + 'px';
			},
			moveScrop: document
		});
	...
	scrollBarFx.prototype.xdrag = function () {
		...
		mDrag({
			...
			disableY: true,
			moveFn() {
				_this.x_barPos = _opts.x_bar.offsetLeft;
				_opts.contentArea.style.left = -_this.x_barPos / _this.x_barRatio + 'px';
			},
			moveScrop: document
		});
	};
